    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                // MpDto.java
                String dtoName = tableInfo.getEntityName() + "DTO";
                if (null != pathInfo.get(ConstVal.DTO_PATH)) {
                    String entityFile = String.format((pathInfo.get(ConstVal.DTO_PATH) + File.separator + "%s" + suffixJavaOrKt()), dtoName);
                    if (isCreate(FileType.DTO, entityFile)) {
                        writer(objectMap, templateFilePath(template.getDto()), entityFile);
                    }
                }
                // Mp.java
                String vmName = tableInfo.getEntityName() + "VM";
                if (null != entityName && null != pathInfo.get(ConstVal.VM_PATH)) {
                    String entityFile = String.format((pathInfo.get(ConstVal.VM_PATH) + File.separator + "%s" + suffixJavaOrKt()), vmName);
                    if (isCreate(FileType.VM, entityFile)) {
                        writer(objectMap, templateFilePath(template.getVm()), entityFile);
                    }
                }
                // MpMapper.java
                ...
                // MpDtoMapper.java
                if (null != tableInfo.getDtoMapperName() && null != pathInfo.get(ConstVal.DTO_MAPPER_PATH)) {
                    String mapperFile = String.format((pathInfo.get(ConstVal.DTO_MAPPER_PATH) + File.separator + tableInfo.getDtoMapperName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.DTO_MAPPER, mapperFile)) {
                        writer(objectMap, templateFilePath(template.getDtoMapper()), mapperFile);
                    }
                }
